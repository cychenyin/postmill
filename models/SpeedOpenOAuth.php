...
/**
 * php sdk for speed.server.com (using OAuth2)
 */
...
/**
 * SPEED OPEN OAuth 认证类(OAuth2)
 * @version 1.0
 */
class SpeedOpenOAuth extends \yii\base\Object {
	/**
	 * @ignore
	 */
	public $client_id;
	 ...
	 * @ignore
	 */
	public static $host_online 	= "http://api.speed.server.com/";
	public static $host_offline	= "http://apitest.speed.server.com/";
	/**
	 * Set timeout default.
	 *
	 * @ignore
	 ...
	 * @ignore
	 */
	public $useragent = 'SPEED OPEN OAuth2 v0.1';
	...
	function statusURL()  { return $this->getApiHost() . 'oauth/statuses'; }
	/**
	 * Set API URLS
	 */
	/**
	 * @ignore
	 */
	function accessTokenURL()  { return $this->getApiHost() . 'oauth/access_token'; }
	/**
	 * @ignore
	 */
	function authorizeURL()    { return $this->getApiHost() . 'oauth/authorize'; }
	...
	public function getApiHost(){
		//环境
		// defined('SERVER_ONLINE') || define("SERVER_ONLINE", TRUE);
		if (SERVER_ONLINE) {
			return self::$host_online;
		}else{
	 ...
	 * authorize接口
	 *
	 * 对应API：{@link http://open.speed.server.com/wiki/base_oauth_authorize}
	 *
	 ...
	 * access_token接口
	 *
	 * 对应API：{@link http://open.speed.server.com/wiki/base_access_token}
	 *
	...
	public function getAccessToken( $type = 'code', $keys ) {
		...
		if ( is_array($token) && !isset($token['error']) && in_array("access_token", $token) ) {
	...
	function getUserInfo( $access_token )
	{
		...
		$params['client_id'] = $this->client_id;
		return $this->post( $this->statusURL(), $params);
	}
	 ...
	 * @return mixed
	 */
	private function post($url, $parameters = array(), $multi = false) {
	...
	public function oAuthRequest($url, $method, $parameters, $multi = false) {
		...
		if (strrpos($url, 'http://') !== 0 && strrpos($url, 'https://') !== 0) {
			$url = $this->getApiHost() . "{$url}.{$this->format}";
		}
